...
var (
	userColumns               = []string{"user_id", "discord_id", "name"}
	userColumnsWithoutDefault = []string{"discord_id", "name"}
...
// BookerBookingsG retrieves all the booking's bookings via booker_id column.
func (o *User) BookerBookingsG(mods ...qm.QueryMod) bookingQuery {
	return o.BookerBookings(boil.GetDB(), mods...)
}

// BookerBookings retrieves all the booking's bookings with an executor via booker_id column.
func (o *User) BookerBookings(exec boil.Executor, mods ...qm.QueryMod) bookingQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"a\".\"booker_id\"=?", o.UserID),
	)

	query := Bookings(exec, queryMods...)
	queries.SetFrom(query.Query, "\"bookings\" as \"a\"")
	return query
}

// DemoUsersG retrieves all the demo_user's demo users.
func (o *User) DemoUsersG(mods ...qm.QueryMod) demoUserQuery {
	return o.DemoUsers(boil.GetDB(), mods...)
}

// DemoUsers retrieves all the demo_user's demo users with an executor.
func (o *User) DemoUsers(exec boil.Executor, mods ...qm.QueryMod) demoUserQuery {
	queryMods := []qm.QueryMod{
		qm.Select("\"a\".*"),
	}
...
// LoadBookerBookings allows an eager lookup of values, cached into the
// loaded structs of the objects.
func (userL) LoadBookerBookings(e boil.Executor, singular bool, maybeUser interface{}) error {
	var slice []*User
	var object *User
	...
	query := fmt.Sprintf(
		"select * from \"bookings\" where \"booker_id\" in (%s)",
	...
	if singular {
		object.R.BookerBookings = resultSlice
		return nil
	}

	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.UserID == foreign.BookerID.Int {
				local.R.BookerBookings = append(local.R.BookerBookings, foreign)
				break
			}
		}
	}
...
// loaded structs of the objects.
func (userL) LoadDemoUsers(e boil.Executor, singular bool, maybeUser interface{}) error {
	var slice []*User
	var object *User
	...
	query := fmt.Sprintf(
		"select * from \"demo_users\" where \"user_id\" in (%s)",
		strmangle.Placeholders(dialect.IndexPlaceholders, count, 1, 1),
	)
	if boil.DebugMode {
		fmt.Fprintf(boil.DebugWriter, "%s\n%v\n", query, args)
...
// AddBookerBookings adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.BookerBookings.
// Sets related.R.Booker appropriately.
func (o *User) AddBookerBookings(exec boil.Executor, insert bool, related ...*Booking) error {
	var err error
	for _, rel := range related {
		rel.BookerID.Int = o.UserID
		rel.BookerID.Valid = true
		if insert {
			if err = rel.Insert(exec); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
			if err = rel.Update(exec, "booker_id"); err != nil {
	...
	if o.R == nil {
		o.R = &userR{
			BookerBookings: related,
		}
	} else {
		o.R.BookerBookings = append(o.R.BookerBookings, related...)
	}
...
// SetBookerBookings removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Booker's BookerBookings accordingly.
// Replaces o.R.BookerBookings with related.
// Sets related.R.Booker's BookerBookings accordingly.
func (o *User) SetBookerBookings(exec boil.Executor, insert bool, related ...*Booking) error {
	query := "update \"bookings\" set \"booker_id\" = null where \"booker_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.BookerBookings {
			rel.BookerID.Valid = false
			if rel.R == nil {
				continue
			}
		...
		o.R.BookerBookings = nil
	}
	return o.AddBookerBookings(exec, insert, related...)
}

// RemoveBookerBookings relationships from objects passed in.
// Removes related items from R.BookerBookings (uses pointer comparison, removal does not keep order)
// Sets related.R.Booker.
func (o *User) RemoveBookerBookings(exec boil.Executor, related ...*Booking) error {
	var err error
	for _, rel := range related {
		rel.BookerID.Valid = false
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.BookerBookings {
			if rel != ri {
				continue
			}

			ln := len(o.R.BookerBookings)
			if ln > 1 && i < ln-1 {
				o.R.BookerBookings[i] = o.R.BookerBookings[ln-1]
			}
			o.R.BookerBookings = o.R.BookerBookings[:ln-1]
			break
		}
	}
...
func (o *User) AddDemoUsers(exec boil.Executor, insert bool, related ...*DemoUser) error {
	var err error
	for _, rel := range related {
...
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.User's DemoUsers accordingly.
// Replaces o.R.DemoUsers with related.
// Sets related.R.User's DemoUsers accordingly.
func (o *User) SetDemoUsers(exec boil.Executor, insert bool, related ...*DemoUser) error {
	query := "update \"demo_users\" set \"user_id\" = null where \"user_id\" = $1"
	values := []interface{}{o.UserID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
...
// Sets related.R.User.
func (o *User) RemoveDemoUsers(exec boil.Executor, related ...*DemoUser) error {
	var err error
	for _, rel := range related {
		rel.UserID.Valid = false
		if rel.R != nil {
